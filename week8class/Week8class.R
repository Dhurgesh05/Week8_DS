...
lower_ex<-first_q - 1.5 * iqr #this produces 17.375
upper_ex<-third_q + 1.5 * iqr #this produces 40.375


#handling outlier 
#drop outlier
data_new<-data
data_new <- data_new[!data_new<lower_ex]
data_new <- data_new[!data_new>upper_ex]
data_new

#Replace the values with mean
data_new <- data
avg <- round(mean(data_new)) #for the purpose of example we round up value
data_new[data_new<lower_ex] <- avg
data_new[data_new>upper_ex] <- avg
data_new
